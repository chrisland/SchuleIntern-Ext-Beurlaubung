      ...
      <table class="si-table" v-if="sortList && sortList.length >= 1">
        ...
        <tr v-bind:key="index" v-for="(item, index) in  sortList"
          ...
          <td v-if="item.status == 1">
            <button class="si-btn si-btn-green margin-r-m" @click="handlerPage(item, 2)"><i class="fa fa-check"></i> Genehmigen</button>
            <button class="si-btn si-btn-red" @click="handlerPage(item, 3)"><i class="fa fa-ban"></i> Nicht Genehmigen</button>
          </td>
...
export default {
  ...
  methods: {
    ...
    loadList() {
      ...
      this.loading = true;
      var that = this;
      axios.get(this.apiURL + '/getListAdmin/open')
          .then(function (response) {
            if (response.data) {
              if (response.data.error) {
                that.error = '' + response.data.msg;
